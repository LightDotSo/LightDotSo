...
- If using the deposited funds in the `LightVault` for funds fronting, the fronted funds will have additional constraints (e.g. can only be transferred w/ the right permissions or policy)
- The chain agnostic operations (can be verified using `LightDAG` contract) must adhere to the protocol specs for orchestration.
