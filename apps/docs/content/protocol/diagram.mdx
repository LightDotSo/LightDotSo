...
- Deposited shares need to be directly delegated to the protocol operator for immediate use in the protocol

## Key Realizations

- **Capital Efficiency**: Both users and solvers need to deposit into the vault to use as bond(escrow) to the protocol for filling operations, why not make a common entry point for both to enable maximum capital efficiency for all parties?
