...
A factory contract for `Lightallet`
...
```solidity
contract Lightallet accountImplementation
```
...
```solidity
function createAccount(address owner, uint256 salt) public returns (contract Lightallet ret)
```
