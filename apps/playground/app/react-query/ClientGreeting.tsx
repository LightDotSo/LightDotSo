...
export function AuthThing() {
  const [me] = api.playground.me.useSuspenseQuery();
...
export function ClientGreeting() {
  const [greeting1] = api.playground.greeting.useSuspenseQuery({
    text: "from react1",
  });
  const [greeting2] = api.playground.greeting.useSuspenseQuery({
    text: "from react2",
  });
  const [secret] = api.playground.secret.useSuspenseQuery();
  ...
  return (
    <div className="space-y-2">
      ...
      <Button onClick={() => trpcContext.playground.greeting.invalidate()}>
        Revalidate HTTP
      </Button>
    </div>
  );
