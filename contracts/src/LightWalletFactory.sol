...
/// @dev A UserOperations "initCode" holds the address of the factory, and a method call (to
/// createAccount, in this sample factory).
/// The factory's createAccount returns the target account address even if it is already installed.
/// This way, the entryPoint.getSenderAddress() can be called either before or after the account is
/// created.
contract LightWalletFactory is ILightWalletFactory {
    ...
    /// @dev Note that during UserOperation execution, this method is called only if the account is
    /// not deployed.
    /// This method returns an existing account address so that entryPoint.getSenderAddress() would
    /// work even after account creation
    ...
    /// @notice calculate the counterfactual address of this account as it would be returned by
    /// createAccount()
    /// @param hash The hash of the account image.
    /// @param salt The salt of the create2 call.
