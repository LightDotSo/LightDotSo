...
interface ILightWalletFactory {
    ...
    function createAccount(bytes32 _hash, uint256 _salt) external returns (LightWallet);

    /// @notice Calculates the counterfactual address of an account
    function getAddress(bytes32 _hash, uint256 _salt) external view returns (address);
}
