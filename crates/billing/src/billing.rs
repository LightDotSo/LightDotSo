...
impl Billing {
        ...
        loop {
            ...
            match result {
                Ok(block) => {
                    ...
                    // trace!("Billing run, chain_id: {} timestamp: {}", chain_id, now);

                    // Info if the second is divisible by 30 or 30 + 1.
                    if now.second() % 30 == 0 || now.second() % 30 == 1 {
                        info!(
                            "Billing run, chain_id: {} timestamp: {} url: {}",
                            chain_id, now, url
                        );
                    }
                    ...
                    if !self.live && min_block >= initial_min_block {
                        warn!(
                            "Billing exiting, chain_id: {} min_block: {} initial_min_block: {} at url: {}",
    ...
    async fn poll_task(&self, chain_id: u64, url: String, mut min_block: i32) -> Result<i32> {
        // Get the billing metrics, set the attempt.
        BillingMetrics::set_attempt(chain_id);
        ...
        if let Some(d) = data {
            ...
            trace!(
                "Billing run, chain_id: {} min_block: {} index: {} user_operations: {:?}",
                chain_id,
                min_block,
                index,
                user_operations
