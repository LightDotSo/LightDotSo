...
pub fn get_pack(user_operation: UserOperationConstruct) -> Token {
    ...
    let push = if (user_operation.init_code.len()) % 32 == 0 ||
        (user_operation.call_data.len()) % 32 == 0
    {
        0
    } else {
        31
    ...
    Token::Bytes(encode(&[
        ...
        Token::Uint(
            // 0x1a0 = 416
            (416 + (((push + user_operation.init_code.len() + user_operation.call_data.len()) /
                32) *
                32))
            .into(),
        ),
        Token::Uint(
            // 0x260 = 608
            (608 + (((push + user_operation.init_code.len() + user_operation.call_data.len()) /
                32) *
                32))
            .into(),
        ),
